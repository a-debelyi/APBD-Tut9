...
BEGIN  
 ...
 DECLARE
@IdProductFromDb INT, @IdOrder INT, @Price DECIMAL(5,2);
...
FROM Product
WHERE IdProduct = @IdProduct IF @IdProductFromDb IS NULL
BEGIN  
  RAISERROR
('Invalid parameter: Provided IdProduct does not exist', 18, 0);  
  RETURN;
END;  
...
BEGIN  
  RAISERROR
('Invalid parameter: There is no order to fullfill', 18, 0);  
  RETURN;
END;  
   
 IF
NOT EXISTS(SELECT 1 FROM Warehouse WHERE IdWarehouse=@IdWarehouse)
BEGIN  
  RAISERROR
('Invalid parameter: Provided IdWarehouse does not exist', 18, 0);  
  RETURN;
END;  
